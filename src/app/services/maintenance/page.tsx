...
export const metadata: Metadata = {
  title: '통합 유지보수',
  description:
    'UTM·WAF·NAC부터 네트워크/서버/스토리지/애플리케이션까지 — 22년 경력의 전문 인력이 24/365 통합 관제.',
};

type ScopeIconName = 'shield' | 'network' | 'server' | 'app';

type Scope = {
  title: string;
  desc: string;
  iconKey: ScopeIconName;
};
    ...
    <div className="space-y-12 pb-10">
      <header className="max-w-4xl">
        ...
        <p className="text-slate-500 font-medium text-sm leading-relaxed">
          솔모정보기술은 22년의 신뢰와 전문 인력을 바탕으로 고객사의 통합
          인프라를 선제적으로 관리하고 신속한 장애 대응을 통해 최상의 가용성을
          제공합니다.
        </p>
      </header>
      ...
      <section className="bg-white border border-slate-200 rounded-3xl p-6 sm:p-10">
        <div className="mb-8">
          ...
          <p className="text-sm font-medium text-slate-500 leading-relaxed">
            그룹웨어부터 네트워크·보안·서버·애플리케이션까지 전 영역을 한 팀이
            통합 관리합니다.
          </p>
        </div>
...
function ScopeIcon({ name }: { name: ScopeIconName }) {
  const props = {
    width: 22,
    height: 22,
    viewBox: '0 0 24 24',
...
function StrengthCard({
  ...
  return (
    <div className="p-7 bg-white/[0.04] backdrop-blur-sm border border-white/10 rounded-2xl hover:bg-white/[0.08] hover:border-blue-400/30 transition-all">
      <div className="text-2xl font-extrabold text-blue-400 mb-4 tracking-tighter">
        {num}
      </div>
        ...
        {SATELLITES.map((s) => (
          <g key={s.label}>
            <circle
              ...
              strokeWidth="2"
            />
            <text x={s.cx} y={s.cy - 6} textAnchor="middle" fontSize="22">
              {s.emoji}
            </text>
            <text
              x={s.cx}
...
const SATELLITES = [
  // (cx,cy) center of icon. (dx,dy) line start offset, (tx,ty) line end offset (toward center hub)
  { cx: 500, cy: 70, emoji: '📋', label: '그룹웨어', dx: 0, dy: 48, tx: 0, ty: 100 },
  { cx: 730, cy: 150, emoji: '🛡', label: '보안 시스템', dx: -33, dy: 34, tx: -145, ty: 90 },
  { cx: 820, cy: 270, emoji: '🌐', label: '네트워크', dx: -48, dy: 0, tx: -220, ty: 0 },
  { cx: 730, cy: 390, emoji: '🗄', label: '서버 · 스토리지', dx: -33, dy: -34, tx: -145, ty: -90 },
  { cx: 500, cy: 470, emoji: '⚙️', label: '애플리케이션', dx: 0, dy: -48, tx: 0, ty: -100 },
  { cx: 270, cy: 390, emoji: '💾', label: '백업 · 복구', dx: 33, dy: -34, tx: 145, ty: -90 },
  { cx: 180, cy: 270, emoji: '🚨', label: '장애 관리', dx: 48, dy: 0, tx: 220, ty: 0 },
  { cx: 270, cy: 150, emoji: '🔄', label: '변경 관리', dx: 33, dy: 34, tx: 145, ty: 90 },
] as const;
