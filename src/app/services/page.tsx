...
export const metadata: Metadata = {
  title: '서비스 개요',
  description:
    '솔모정보기술의 보안 서비스 — 보안취약점 분석, 보안프린트 구축, 통합 유지보수.',
};
  ...
  return (
    <div className="bg-white min-h-screen pt-32 pb-24">
      <div className="bg-slate-900 py-32 relative overflow-hidden mb-24">
        ...
        <div className="max-w-[1400px] mx-auto px-8 relative z-10">
          ...
          <h1 className="text-5xl md:text-7xl font-black text-white leading-tight mb-8 tracking-tighter">
            비즈니스의 안전을 책임지는 <br />
            <span className="text-blue-500 underline decoration-blue-500 decoration-8 underline-offset-16">
              토탈 보안 서비스.
            </span>
          </h1>
          <p className="text-white/40 text-xl font-medium max-w-3xl leading-relaxed">
            단순한 솔루션 공급을 넘어 전문 기술력과 22년의 노하우로 <br />
            귀사의 IT 인프라 가용성과 보안성을 극대화합니다.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-8 space-y-40">
        <section
          id="vulnerability"
          className="grid lg:grid-cols-2 gap-24 items-center scroll-mt-32"
        >
          <div>
            <div className="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center text-3xl mb-8">
              🛡️
            </div>
            ...
            <p className="text-slate-500 font-bold leading-relaxed text-lg mb-12">
              정보자산에 대한 잠재적 위협을 사전에 파악하여 완벽한 보안 대책을
              수립합니다. 웹/앱 서비스 및 내부 인프라의 취약점을 분석하여 해킹
              사고를 미연에 방지합니다.
            </p>

            <div className="space-y-6">
              <ExpertBullet
                title="모의해킹 (Penetration Testing)"
                desc="외부 전문가 관점에서 실제 해킹 시나리오를 통한 침투 테스트 수행"
              />
              <ExpertBullet
                title="취약점 정밀 진단"
                desc="웹 서버, DB, 네트워크 장비 등의 설정 오류 및 보안 약점 점검"
              />
              <ExpertBullet
                title="소스코드 진단 (Secure Coding)"
                desc="개발 단계에서의 취약점 제거를 위한 소스코드 분석"
              />
            </div>
            ...
            <div className="mt-12 p-8 bg-slate-50 rounded-3xl border border-slate-100 flex items-center gap-8">
              ...
              <p className="text-xs font-bold text-slate-400">
                ISMS, 개인정보보호법 등 법적 요구사항 <br />
                준수 점검 및 법적 리스크 완벽 해소
              </p>
            </div>
          </div>
        ...
        >
          <div className="lg:order-2">
            ...
            <p className="text-slate-500 font-bold leading-relaxed text-lg mb-12">
              종이 문서를 통한 정보 유출을 원천 차단하는 스마트 보안 출력
              솔루션입니다. 자체 스크린 워터마크 기술 xSecuritas 기반으로 강력한
              추적 체계를 구축합니다.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <MiniCard
                title="출력 워터마크 삽입"
                desc="인쇄 시 사용자 ID, IP, 날짜 등을 자동 삽입하여 유출 경로 추적 가능"
              />
              <MiniCard
                title="출력 승인 및 통제"
                desc="민감 정보 포함 시 관리자 승인 절차를 통한 인쇄물 통제"
              />
              <MiniCard
                title="상세 이력 감사"
                desc="누가, 언제, 무엇을 출력했는지에 대한 실시간 로그 기록 유지"
              />
              <MiniCard
                title="인프라 연동"
                desc="기존 프린터 환경 유지하며 보안 모듈만 유연하게 연동 가능"
              />
            </div>
          </div>
        ...
        <section
          id="maintenance"
          className="bg-[#001F5B] rounded-[4rem] p-16 md:p-32 text-white relative overflow-hidden scroll-mt-32"
        >
          ...
          <div className="relative z-10 flex flex-col lg:flex-row gap-20 items-center">
            <div className="max-w-2xl">
              ...
              <p className="text-white/50 font-bold leading-relaxed text-xl mb-12">
                단순 장애 처리를 넘어 정기 점검, 성능 최적화, 보안 업데이트 등
                선제적 관리를 수행하여 24/365 안정적 운영을 보장합니다.
              </p>
            ...
            <div className="w-full lg:flex-1 space-y-4">
              <MaintenanceRow
                label="보안 시스템"
                value="UTM, 웹 방화벽(WAF), NAC, DLP 등 전반"
              />
              <MaintenanceRow
                label="네트워크"
                value="L2/L3/L4 스위치 및 무선랜 최적화 관리"
              />
              <MaintenanceRow
                label="서버 인프라"
                value="UNIX, Linux, Windows 서버 및 백업 관리"
              />
            </div>
          </div>
        </section>
      ...
      <section className="mt-40 text-center py-20 bg-slate-50 border-t border-slate-100">
        <h3 className="text-3xl font-black text-slate-900 mb-8">
          상세 보안 서비스 컨설팅이 필요하신가요?
        </h3>
        <Link
          href="/support/contact"
          className="inline-block px-12 py-5 bg-[#001F5B] text-white font-black rounded-full hover:bg-blue-900 transition-all shadow-xl shadow-blue-900/20 active:scale-95"
        >
          전문 엔지니어 상담하기
        </Link>
      </section>
...
function ExpertBullet({ title, desc }: { title: string; desc: string }) {
  return (
    <div className="flex items-start gap-4 p-5 rounded-2xl hover:bg-slate-50 transition-all group">
      <div className="w-8 h-8 rounded-lg bg-blue-600 flex-shrink-0 flex items-center justify-center text-white font-bold group-hover:scale-110 transition-transform">
        ✓
      </div>
      <div>
  ...
  return (
    <div>
      <div className="text-4xl font-black text-blue-400 tracking-tighter mb-1">
        {value}
        <span className="text-sm ml-1 opacity-50 uppercase">{unit}</span>
      </div>
