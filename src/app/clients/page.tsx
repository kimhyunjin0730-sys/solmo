...
const LOGO = (n: number) =>
  `/images/client/${encodeURIComponent(`그림${n}.png`)}`;

type ClientCategory = '금융' | '기업/제조' | '공공' | '교육/병원';
type Client = {
  name: string;
  nameEn: string;
...
type FilterCategory = '전체' | ClientCategory;
...
const CATEGORY_META: Record<ClientCategory, CategoryMeta> = {
  금융: {
    color: 'bg-blue-600',
    ring: 'ring-blue-600',
    text: 'text-blue-600',
...
export default function ClientsPage() {
  ...
  const counts = useMemo<Record<FilterCategory, number>>(() => {
    const acc = { 전체: CLIENTS.length } as Record<FilterCategory, number>;
    for (const cat of CATEGORIES.slice(1) as ClientCategory[]) {
      acc[cat] = CLIENTS.filter((c) => c.category === cat).length;
    }
    return acc;
  }, []);
        ...
        <div className="max-w-[1400px] mx-auto px-5 sm:px-8 py-14 sm:py-20">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between mb-10 sm:mb-12 gap-4 lg:gap-6">
            ...
            <p className="text-xs sm:text-sm font-bold text-slate-400 max-w-md">
              산업군별 누적 고객사 비중. 각 영역을 탭하면 해당 카테고리로
              필터링됩니다.
            </p>
          </div>

          <div className="w-full flex h-4 rounded-full overflow-hidden mb-8">
            {(CATEGORIES.slice(1) as ClientCategory[]).map((cat) => {
        ...
        {filtered.length > 0 ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm font-black text-slate-400 uppercase tracking-widest">
              &ldquo;{query}&rdquo; 에 해당하는 고객사가 없습니다.
            </p>
          </div>
        )}
      </section>
...
function LogoCard({ client }: { client: Client }) {
  ...
  return (
    <div className="group bg-white rounded-2xl border border-slate-100 hover:border-slate-300 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 p-4 sm:p-6 flex flex-col items-center relative">
      <div
        className={`absolute top-3 right-3 w-1.5 h-1.5 rounded-full ${meta.color} opacity-60`}
      />
